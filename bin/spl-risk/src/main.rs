...
async fn main() -> Result<()> {
    ...
    if cli.cache_stats {
        ...
        println!("  Token Cache: {}/{} entries ({} expired)", 
            stats.token_cache.size, 
            stats.token_cache.capacity,
            stats.token_cache.expired_entries
        );
        println!("  Metadata Cache: {}/{} entries ({} expired)", 
            stats.metadata_cache.size,
            stats.metadata_cache.capacity,
            stats.metadata_cache.expired_entries
        );
        println!("  Wallet Age Cache: {}/{} entries ({} expired)",
