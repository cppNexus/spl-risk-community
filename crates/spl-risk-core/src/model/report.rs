...
impl RiskReport {
    ...
    pub fn generate_summary(&mut self) {
        let base_summary = match self.risk_score {
            ...
            61..=80 => "High risk. Significant red flags present. High probability of issues.".to_string(),
        ...
        if self.flags.mint_authority && self.flags.freeze_authority {
            self.summary.push_str(" Token owner retains destructive privileges.");
        }
        
        // Add warning if holder data is incomplete
        if self.metrics.holders == 0 && (self.data_sources.holders == "partial" || self.data_sources.holders == "timeout") {
            ...
            self.summary.push_str(" Note: Unable to fetch complete holder data.");
        }
